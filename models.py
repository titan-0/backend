from sqlalchemy import Column, String, Integer, Boolean, Numeric, Date, BigInteger, Enum, DateTime, func, ForeignKey, Index, Text, Time, SmallInteger, Float
...
class InternalOrder(Base):
    ...
    ordertype = Column(
        Enum(
            "InternalOrder","InternalBO","BO_TWAP_Timed_SLL","InternalRegularOvernight",
            "DummyInternalBO","InternalRegularOrder","PositionalOrder","IRMarketOrder",
            "IRO_TWAP_Timed_SLL","IPaperOrder","IProxyOrder","SingleLegOrder","ITradeOrder",
            "IRTrigger","PositionalPaper",
            name="ordertype_enum"
        ),
        nullable=False
    )
    ...
    ticksize = Column(Float, default=0.05)

    broker_order_tag = Column(BigInteger)
    broker_parent_order_id = Column(Integer)

    twap_leg_count = Column(SmallInteger, default=0)
    twap_leg_no = Column(SmallInteger, default=0)

    time_in_force = Column(Time)
    ...
    time_in_force_exit_SL = Column(Time)
    time_in_force_exit_TP = Column(Time)
    ...
    # Virtual columns - generated by database, read-only
    order_date = Column(Date)
    last_updated_date = Column(Date)


# Indexes for InternalOrder
Index("ix_last_updated_internalorder", InternalOrder.last_updated)
Index("ix_ordertime_internalorder", InternalOrder.ordertime)
Index("ix_last_updated_broker_internalorder", InternalOrder.last_updated, InternalOrder.broker)
Index("ix_last_updated_brokeraccount_internalorder", InternalOrder.last_updated, InternalOrder.brokeraccount)
Index("ix_last_updated_client_id_internalorder", InternalOrder.last_updated, InternalOrder.client_id)
Index("ix_last_updated_dealer_id_internalorder", InternalOrder.last_updated, InternalOrder.dealer_id)
Index("ix_last_updated_strategy_id_internalorder", InternalOrder.last_updated, InternalOrder.strategy_id)
Index("ix_last_upfated_ticker_internalorder", InternalOrder.last_updated, InternalOrder.ticker)
Index("ix_internalorder_signal_id", InternalOrder.signal_id)
...
class BrokerOrder(Base):
    ...
    # Virtual column - generated by database
    orderdate = Column(Date)


# Indexes for BrokerOrder
Index("ix_order_timestamp_brokerorder", BrokerOrder.order_timestamp)
Index("ix_order_timestamp_brokeraccount_brokerorder", BrokerOrder.order_timestamp, BrokerOrder.brokeraccount)
...
Index("ix_order_timestamp_broker_client_id", BrokerOrder.order_timestamp, BrokerOrder.broker, BrokerOrder.client_id)
...
# Indexes for Trade
Index("ix_exchange_timestamp_trade", Trade.exchange_timestamp)
Index("ix_exchange_timestamp_broker", Trade.exchange_timestamp, Trade.broker)
Index("ix_exchange_timestamp_brokeraccount", Trade.exchange_timestamp, Trade.brokeraccount)
Index("ix_exchange_timestamp_client_id", Trade.exchange_timestamp, Trade.client_id)
Index("ix_exchange_timestamp_ticker_trade", Trade.exchange_timestamp, Trade.ticker)
